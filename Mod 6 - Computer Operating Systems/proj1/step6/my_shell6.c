...
void interactive(){ //my_shell3 main program, which itself uses my_shell2
  ...
  while ((nread = getline(&line_buf, &len, stdin)) != -1) {
    ...
    else if(strcmp(my_args[0], "my_ls") == 0){ //my_ls simplified ls -l ("usage: my_ls, with no additional parameters")
    ...
    else if(strcmp(my_args[0], "my_cp") == 0){ //my_cp inputfile outputfile (overrides existing file if already exists) (error if not right parameters "usage: my_cp input_file output_file")
